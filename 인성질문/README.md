...
나의 단점은 뭐다 -> 정확히 인지하고 있다 -> 어떤 노력을 하고 있다 -> 개선이 되고 있다는 얘기를 듣는다.<br>
종종 성격이 급해지는 것이 저의 단점입니다. 주어진 일은 빨리 처리해야 하고, 일이 쌓이는 것을 잘 보지 못합니다.
그렇다보니 가끔은 한두가지를 빠뜨리고 일을 처리해버리는 실수를 범하기도 했습니다.
실수를 저지르다보니 저는 결과물에 실망하게 되었고 이를 고치기 위해 모든 일에 우선순위를 세우고 마지막에 항상 저체적으로 검토하는 습관이 생겼습니다. 
급한 성격인 만큼 업무를 기한내에 빨리 처리하는것도 중요하지만, 더 정확하고 확실하게 처리해야 한다는 것을 알기에 저 단점을 보완할 수 있었습니다.
...
### 간단한 1분 자기소개 부탁드려요.
안녕하세요. 배우는 자세로 새로운 것을 스펀지처럼 흡수하려고 노력하는 지원자 류지수입니다. 저는 대학교 4학년때 제조 데이터를 다루는 스타트업에서 인턴십을 진행한 적이 있습니다. 인턴십을 하면서 프로젝트를 통해 현재 문제를 해결하는 경험을 할 수 있었습니다. 프로젝트를 진행하면서 모델에 대한 이해도를 높이려고 노력했으나, 아직 제가 부족하다는 것을 깨달았고 인턴십이 끝난 후 바로 취업 준비를 하지 않고 네이버에서 진행한 부스트캠프를 들었습니다. 부스트캠프를 통해 개념을 쌓을 수 있었고, 팀원들과 깃허브 활용을 통해 협업을 배울 수 있었습니다. 제가 경험한 것들과 쌓은 지식을 통해 데이터를 파악하고 절절한 모델을 사용할 수 있는 개발자가 되고싶습니다.
